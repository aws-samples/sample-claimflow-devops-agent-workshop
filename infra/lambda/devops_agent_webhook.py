...
Triggered when any ClaimFlow alarm enters ALARM state.
...
def lambda_handler(event, context):
    """
    Triggered by CloudWatch Alarm state change (via SNS).
    ...
    if not WEBHOOK_URL or not webhook_secret:
        print("ERROR: WEBHOOK_URL and WEBHOOK_SECRET_ARN must both be configured")
    ...
    try:
        ...
        # Create DevOps Agent webhook payload
        payload = {
            "eventType": "incident",
            ...
            "timestamp": timestamp,
            "service": alarm_name.replace("ClaimFlow-", ""),
            "data": {
                "metadata": {
