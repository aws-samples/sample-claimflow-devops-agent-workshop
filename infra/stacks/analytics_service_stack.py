"""Analytics Service Stack — ECS Fargate service with Aurora and QuickSight access."""

from aws_cdk import (
    Stack,
    aws_ec2 as ec2,
    aws_ecs as ecs,
    aws_iam as iam,
    aws_logs as logs,
    aws_rds as rds,
    aws_servicediscovery as sd,
)
from constructs import Construct
from shared_constructs.fargate_service import FargateServiceConstruct


class AnalyticsServiceStack(Stack):
    """Analytics Service — data aggregation and QuickSight dashboard integration."""

    def __init__(
        self,
        scope: Construct,
        construct_id: str,
        config: dict,
        vpc: ec2.Vpc,
        cluster: ecs.Cluster,
        security_group: ec2.SecurityGroup,
        cloud_map_namespace: sd.PrivateDnsNamespace,
        log_group: logs.LogGroup,
        aurora_cluster: rds.DatabaseCluster,
        claims_table_arn: str,
        **kwargs,
    ) -> None:
        super().__init__(scope, construct_id, **kwargs)

        ecs_config = config["ecs"]
        sd_namespace = config["service_discovery"]["namespace"]

        self.service_construct = FargateServiceConstruct(
            self,
            "AnalyticsService",
            cluster=cluster,
            service_name="analytics-service",
            docker_image_path="../services/analytics-service",
            vpc=vpc,
            security_group=security_group,
            cloud_map_namespace=cloud_map_namespace,
            log_group=log_group,
            cpu=ecs_config["cpu"],
            memory_limit_mib=ecs_config["memory_limit_mib"],
            desired_count=ecs_config["desired_count"],
            min_capacity=ecs_config["min_capacity"],
            max_capacity=ecs_config["max_capacity"],
            cpu_scaling_target=ecs_config["cpu_scaling_target"],
            environment={
                "AURORA_HOST": aurora_cluster.cluster_endpoint.hostname,
                "AURORA_PORT": "5432",
                "AURORA_DATABASE": config["aurora"]["database_name"],
                "AURORA_SECRET_ARN": aurora_cluster.secret.secret_arn,
                "CLAIMS_TABLE": f"{config['project_name']}-claims",
                "AUTH_SERVICE_URL": f"http://auth-service.{sd_namespace}:8000",
            },
        )

        # IAM permissions — DynamoDB read (for data sync)
        self.service_construct.task_role.add_to_policy(
            iam.PolicyStatement(
                actions=[
                    "dynamodb:GetItem",
                    "dynamodb:Query",
                    "dynamodb:Scan",
                ],
                resources=[
                    claims_table_arn,
                    f"{claims_table_arn}/index/*",
                ],
            )
        )

        # IAM permissions — Secrets Manager (Aurora credentials)
        self.service_construct.task_role.add_to_policy(
            iam.PolicyStatement(
                actions=["secretsmanager:GetSecretValue"],
                resources=[aurora_cluster.secret.secret_arn],
            )
        )

        # IAM permissions — QuickSight
        self.service_construct.task_role.add_to_policy(
            iam.PolicyStatement(
                actions=[
                    "quicksight:GenerateEmbedUrlForRegisteredUser",
                    "quicksight:GetDashboardEmbedUrl",
                ],
                resources=["*"],
            )
        )
